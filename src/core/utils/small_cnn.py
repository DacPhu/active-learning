...
class SmallCNN(object):
  ...
  def transform(self, X):
    ...
    dense1_layer = next(layer for layer in model.layers if layer.name == 'dense1')

    intermediate_model = tf.keras.models.Model(inputs=model.input, outputs=dense1_layer.output)
    batch_size = self.batch_size
    ...
    for i in range(num_batches):
      ...
      batch_output = intermediate_model(minibatch, training=False)
      activations.append(batch_output.numpy())
    output = np.vstack(activations)
    return output
